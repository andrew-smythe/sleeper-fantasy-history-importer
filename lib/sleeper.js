...
module.exports = {
            ...
            if (result.length > 0) {
                ...
                for (let week = 1; week <= 17; week++) {
                    try {
                        ...
                        // Get roster position info from db
                        let positions = await db.query('SELECT rp.id, lsp.numSlots, rp.position FROM `leaguesettingspositions` `lsp` JOIN `rosterpositions` `rp` on `rp`.`id` = `lsp`.`rosterPositionId` where `lsp`.year = ' + year + ' ORDER BY `rp`.order');
                        let positionSlots = [];

                        for (let p of positions) {
                            for (let i = 0; i < p.numSlots; i++) {
                                positionSlots.push(p.id);
                            }
                        }

                        if (positions.length > 0 && sleeperPlayers.length > 0) {
                            // Collect all player stats and add to database
                            for (let matchup of matchups) {
                                // Add all starters
                                let starters = matchup.starters;
                                let points = matchup.playerPoints;
                                let bench = matchup.bench;

                                for (let i in starters) {
                                    // Get player data from imported Sleeper players
                                    let player = sleeperPlayers.find(sp => sp.sleeperId == starters[i]);
                                    ...
                                    let insertQuery = "INSERT INTO `weeklyplayerstats` (`teamId`, `week`, `name`, `team`, `playerPosition`, `rosterPositionId`, `totalPoints`, `source`) VALUES \
                                                    (" + matchup.teamId + ", " + week + ", '" + nameStr + "', " + teamStr + ", " +  positionStr + ", " + positionSlots[i] + ", " + (points[starters[i]] ? points[starters[i]] : "NULL") + ", 'sleeper')";
                                ...
                                for (let i in bench) {
                                    let player = sleeperPlayers.find(sp => sp.sleeperId == bench[i]);
                                    if (!player) {
                                        console.error('ERROR - Could not find player with Sleeper ID #' + p);
                                        continue;
                                    }
                                    ...
                                    let insertQuery = "INSERT INTO `weeklyplayerstats` (`teamId`, `week`, `name`, `team`, `playerPosition`, `rosterPositionId`, `totalPoints`, `source`) VALUES \
                                                    (" + matchup.teamId + ", " + week + ", '" + nameStr + "', " + teamStr + ", " +  positionStr + ", " + 8 + ", " + (points[bench[i]] ? points[bench[i]] : "NULL") + ", 'sleeper')";
                                    let insertStatsResult = await db.query(insertQuery);
                                    if (!insertStatsResult) {
                                        console.error('ERROR - Could not insert bench player stats for ' + player.name + ' (matchup ID ' + matchup.matchupId + ', week ' + week + ', ' + year + ')');
                                    }
                                }
                            }
                        }
    ...
    importLeagueSettings: async function (db, leagueIds) {
        ...
        for (let league of leagueIds) {
            ...
            console.log('Importing league settings from Sleeper for year ' + year + '...');

            // Check if league settings have already been imported for this year
            result = await db.query("SELECT id FROM leaguesettings WHERE source = 'sleeper' AND year = " + year);
            if (result.length > 0) {
                console.log(year + ' league settings have already been imported.');
                continue;
            }

            try {
                let leagueSettingsUrl = sleeperApiUrl + 'league/' + leagueId;
                let leagueSettingsFetch = await fetch(leagueSettingsUrl);
                await this.sleep();
                if (!leagueSettingsFetch.ok) {
                    throw leagueSettingsFetch.status;
                }

                let rawLeagueSettings = await leagueSettingsFetch.json();
                let leagueSettings = {
                    numTeams: rawLeagueSettings.settings.num_teams,
                    playoffTeams: rawLeagueSettings.settings.playoff_teams,
                    numRegularWeeks: rawLeagueSettings.settings.playoff_week_start - 1,
                    numPlayoffWeeks: (18 - rawLeagueSettings.settings.playoff_week_start),
                    year: year,
                };

                let insertQuery = "INSERT INTO `leaguesettings` (`year`, `numWeeksRegular`, `numWeeksPlayoffs`, `numTeams`, `numTeamsPlayoffs`, `source`) \
                                   VALUES (" + leagueSettings.year + ", " + leagueSettings.numRegularWeeks + ", " + leagueSettings.numPlayoffWeeks + ", " + leagueSettings.numTeams + ", " + leagueSettings.playoffTeams + ", 'sleeper')";

                let result = await db.query(insertQuery);
                if (!result) {
                    console.error('ERROR - Could not insert league settings into DB');
                }
            }
            catch (e) {
                console.error('ERROR - Could not fetch league settings from Sleeper', e);
            }
        }
    },

    importLeagueRosterSettings: async function (db, leagueIds) {
        ...
        for (let league of leagueIds) {
            ...
            console.log('Importing league roster settings from Sleeper for year ' + year + '...');

            // Check if league settings have already been imported for this year

            result = await db.query("SELECT id FROM leaguesettingspositions WHERE year = " + year);
            if (result.length > 0) {
                console.log(year + ' league roster settings have already been imported.');
                continue;
            }
            ...
            try {
                ...
                if (result.length > 0) {
                    ...
                    let rawLeagueRosterSettings = await leagueSettingsFetch.json();
                    for (let pos of positions) {
                        ...
                        rosterSettings.push({
                            id: pos.id,
                            numSlots: rawLeagueRosterSettings.roster_positions.filter(rs => rs === position).length,
                        });
                    }

                    for (let rs of rosterSettings) {
                        let insertQuery = "INSERT INTO `leaguesettingspositions` (`year`, `rosterPositionId`, `numSlots`) \
                                           VALUES (" + year + ", " + rs.id + ", " + rs.numSlots + ")";
                ...
                console.error('ERROR - Could not fetch league roster settings from Sleeper', e);
            }
        }
    }
}